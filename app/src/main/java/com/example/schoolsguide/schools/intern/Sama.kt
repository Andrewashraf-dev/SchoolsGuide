...
import com.example.schoolsguide.databinding.SamaBinding

import com.example.schoolsguide.databinding.StmaryBinding
import com.example.schoolsguide.tabAdapter.MyAdapter
import com.example.schoolsguide.tabAdapter.MyAdapter15
...
private lateinit var binding: SamaBinding

class Sama : AppCompatActivity() ,BaseSliderView.OnSliderClickListener, ViewPagerEx.OnPageChangeListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = SamaBinding.inflate(layoutInflater)
    ...
    private fun getSlider() {
        for (item in 0 until img.count()) {
            ...
            val textSliderView = TextSliderView(this).apply {
                description("Photos")
                image(img[item])
                setOnSliderClickListener(this@Sama)
    ...
    private fun getOpenFacebookIntent(): Intent? {
        return try {
            ...
            Intent(Intent.ACTION_VIEW, Uri.parse("https://www.facebook.com/sama.international.school/?locale=ar_AR"))
        } catch (e: Exception) {
            Intent(
                Intent.ACTION_VIEW,
                Uri.parse("https://www.facebook.com/sama.international.school/?locale=ar_AR")
            )
        }
    }
    fun Apply(view: View) {
