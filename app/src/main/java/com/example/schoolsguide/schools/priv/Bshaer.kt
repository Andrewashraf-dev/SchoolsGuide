...
import com.example.schoolsguide.databinding.BshaerBinding

import com.example.schoolsguide.databinding.StmaryBinding
import com.example.schoolsguide.tabAdapter.MyAdapter
import com.example.schoolsguide.tabAdapter.MyAdapter10
...
private lateinit var binding: BshaerBinding

class Bshaer : AppCompatActivity() ,BaseSliderView.OnSliderClickListener, ViewPagerEx.OnPageChangeListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = BshaerBinding.inflate(layoutInflater)
    ...
    private fun getSlider() {
        for (item in 0 until img.count()) {
            ...
            val textSliderView = TextSliderView(this).apply {
                description("Photos")
                image(img[item])
                setOnSliderClickListener(this@Bshaer)
    ...
    private fun getOpenFacebookIntent(): Intent? {
        return try {
            ...
            Intent(Intent.ACTION_VIEW, Uri.parse("https://www.facebook.com/albashaerinternationalschool/"))
        } catch (e: Exception) {
            Intent(
                Intent.ACTION_VIEW,
                Uri.parse("https://www.facebook.com/albashaerinternationalschool/")
            )
        }
    }
    fun Apply(view: View) {
