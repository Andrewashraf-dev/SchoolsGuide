...
import com.example.schoolsguide.databinding.StgeorgeBinding

import com.example.schoolsguide.databinding.StmaryBinding
import com.example.schoolsguide.tabAdapter.MyAdapter
import com.example.schoolsguide.tabAdapter.MyAdapter12
...
private lateinit var binding: StgeorgeBinding

class StGeorge : AppCompatActivity() ,BaseSliderView.OnSliderClickListener, ViewPagerEx.OnPageChangeListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = StgeorgeBinding.inflate(layoutInflater)
    ...
    private fun getSlider() {
        for (item in 0 until img.count()) {
            ...
            val textSliderView = TextSliderView(this).apply {
                description("Photos")
                image(img[item])
                setOnSliderClickListener(this@StGeorge)
    ...
    private fun getOpenFacebookIntent(): Intent? {
        return try {
            ...
            Intent(Intent.ACTION_VIEW, Uri.parse("https://www.facebook.com/sgis.bns/"))
        } catch (e: Exception) {
            Intent(
                Intent.ACTION_VIEW,
                Uri.parse("https://www.facebook.com/sgis.bns/")
            )
        }
    }
    fun Apply(view: View) {
