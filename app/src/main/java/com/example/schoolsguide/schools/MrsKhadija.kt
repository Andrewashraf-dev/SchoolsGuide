...
import com.example.schoolsguide.databinding.MrskhadijaBinding
import com.example.schoolsguide.tabAdapter.MyAdapter
import com.example.schoolsguide.tabAdapter.MyAdapter4
import com.google.android.material.tabs.TabLayout

private lateinit var tabLayout: TabLayout
private lateinit var viewPager: ViewPager

private lateinit var binding: MrskhadijaBinding

class MrsKhadija : AppCompatActivity() ,BaseSliderView.OnSliderClickListener, ViewPagerEx.OnPageChangeListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = MrskhadijaBinding.inflate(layoutInflater)
    ...
    private fun getSlider() {
        for (item in 0 until img.count()) {
            ...
            val textSliderView = TextSliderView(this).apply {
                description("Photos")
                image(img[item])
                setOnSliderClickListener(this@MrsKhadija)
    ...
    private fun getOpenFacebookIntent(): Intent? {
        return try {
            ...
            Intent(Intent.ACTION_VIEW, Uri.parse("https://www.facebook.com/khadijaschool/"))
        } catch (e: Exception) {
            Intent(
                Intent.ACTION_VIEW,
                Uri.parse("https://www.facebook.com/khadijaschool/")
            )
        }
    }
    fun Apply(view: View) {
