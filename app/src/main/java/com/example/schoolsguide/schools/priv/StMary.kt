...
private lateinit var binding: StmaryBinding

class StMary : AppCompatActivity() ,BaseSliderView.OnSliderClickListener, ViewPagerEx.OnPageChangeListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = StmaryBinding.inflate(layoutInflater)
    ...
    private fun getSlider() {
        for (item in 0 until img.count()) {
            ...
            val textSliderView = TextSliderView(this).apply {
                description("Photos")
                image(img[item])
                setOnSliderClickListener(this@StMary)
    ...
    private fun getOpenFacebookIntent(): Intent? {
        return try {
            ...
            Intent(Intent.ACTION_VIEW, Uri.parse("https://www.facebook.com/StMarySchoolShoubra/"))
        } catch (e: Exception) {
            Intent(
                Intent.ACTION_VIEW,
                Uri.parse("https://www.facebook.com/StMarySchoolShoubra/")
            )
        }
    }
    fun Apply(view: View) {
